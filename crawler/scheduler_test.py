...
class DomainTest(unittest.TestCase):
    ...
    def test_domain(self):
        ...
        domain.accessed_now()
        self.assertTrue(not domain.is_accessible(), 'Como ele acabou de ser acessado, ele não pode estar acessível')
        print('Verificando acesso a um domínio já requisitado (após espera)')
...
class SchedulerTest(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(len(arr_urls_seeds), len(arr_str_urls_seeds), "Nao foi adicionado as sementes solicitadas")
    ...
    def test_add_remove_page(self):
        ...
        # como o primeiro e segundo parâmetro, respectivamente
        ...
        print('Resgatando a segunda página do mesmo domínio...')
    ...
    def test_can_fetch(self):
        ...
        self.assertTrue(obj_url_not_allowed.netloc in self.scheduler.dic_robots_per_domain, msg=f'O domínio <{obj_url_not_allowed.netloc}> não foi encontrado')
        ...
        self.assertTrue(obj_robot_not_allowed == self.scheduler.dic_robots_per_domain[obj_url_not_allowed.netloc], 'Na segunda requisição de um mesmo domínio, você não pode criar um novo objeto RobotFileParser.')

        self.assertTrue(bol_allowed, f'O mesmo robots.txt não pode ser requisitado 2 vezes.')
